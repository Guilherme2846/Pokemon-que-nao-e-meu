	...
	function nx0(){
		p.perm.xpatk[ind]=p.perm.xpatk[ind]+Math.floor(5*Math.random());
		p.perm.xpatkesp[ind]=p.perm.xpatkesp[ind]+Math.floor(5*Math.random());
		p.perm.xpdef[ind]=p.perm.xpdef[ind]+Math.floor(5*Math.random());
		p.perm.xpdefesp[ind]=p.perm.xpdefesp[ind]+Math.floor(5*Math.random());
		p.perm.xphp[ind]=p.perm.xphp[ind]+Math.floor(5*Math.random());
		p.perm.xpspd[ind]=p.perm.xpspd[ind]+Math.floor(5*Math.random());
	}
	function inx0(){
	...
	opbat=0,op=0,ind=0,indcap=0,indpc=0,indi=0,prec=0,din=1000000,pkb=0,gpkb=0,upkb=0,mpkb=0,heg=0,hem=0,hep=0,comp=0,cap=0,lug=0/*l*/,mei=0,meio=0,gr=0,chan=0,mark=0,mud=0,
	esq={nome:0,cat:0,el:0,pp:0,pr:0,pwr:0},dano,time,controle=false,pcontrole=false,gambiarra1=0,gambiarra2=false,gambiarra3=false,gambiarracrit=false,
	...
	//inicio
		...
		function opcionar(event){
			...
			if(voce==estado.selecao){
				...
				if(lug==1 && document.body.querySelector('p#cmm').hidden){
					switch(tecla){
						case "Enter":
						clearTimeout(time);
							if(op==0){								
			...
			}else if(!document.body.querySelector('p#op0').hidden){
				switch(tecla){
					case "0":
						opbat=0;
						document.body.querySelector('p#enter').hidden=false;
